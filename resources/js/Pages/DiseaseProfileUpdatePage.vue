...
const submit = async () => {
    // Reset alerts or previous errors
    alert.value = { show: false, message: '', type: '' };

    // Validate form data (optional, as server-side validation exists)
    if (!validateForm()) {
        alert.value = { show: true, message: 'Please fix the validation errors.', type: 'error' };
        return;
    }

    const payload = {
        disease_id: diseaseProfile.value.disease_id,
        primary_site: form.primary_site,
        custom_primary_site: form.custom_primary_site,
        diagnosis_date: form.diagnosis_date,
        diagnosis_basis: form.diagnosis_basis,
        laterality: form.laterality,
        pathology: form.pathology,
        hist_tumor_extension: form.hist_tumor_extension,
        hist_tumor_grade: form.hist_tumor_grade,
        hist_nodepositive: form.hist_nodepositive,
        hist_nodeharvest: form.hist_nodeharvest,
        hist_negmargins: form.hist_negmargins,
        hist_stage: form.hist_stage,
        extent_of_disease: form.extent_of_disease,
        tumor_size: form.tumor_size,
        nodal_involvement: form.nodal_involvement,
        metastatic_site: form.metastatic_site,
        other_metastatic_site: form.other_metastatic_site,
        metastatic_notes: form.metastatic_notes,
        multiple_primaries: form.multiple_primaries,
        other_primary_sites: form.other_primary_sites,
        additional_primary_site: form.additional_primary_site,
        t_stage: form.t_stage,
        n_stage: form.n_stage,
        m_stage: form.m_stage,
        g_stage: form.g_stage,
        grade: form.grade,
        stage: form.stage,
        stage_type: form.stage_type,
        full_diagnosis: form.full_diagnosis,
        disease_status: form.disease_status,
    }

    try {
        // Send the update request
        const response = await axios.put(`/api/disease-profile/update`, payload);

        // Handle success
        alert.value = { show: true, message: 'Disease profile updated successfully!', type: 'success' };
    } catch (error) {
        // Handle errors
        ...
        if (error.response && error.response.data && error.response.data.errors) {
            ...
            alert.value = { show: true, message: 'Failed to update the disease profile. Check errors.', type: 'error' };
        } else {
            alert.value = { show: true, message: 'Unexpected error occurred.', type: 'error' };
        }
    }
